...
namespace Utility {
	...
	void shiftLeft(const AvxBitArray& bits, AvxBitArray& shiftedBits, bool rightmostBit) {
		...
		shiftedBits = bits >> 1;
		if (rightmostBit) {
			__m256i rightmost = _mm256_set1_epi32(MSB);
			shiftedBits.blend(rightmost, 0b10000000);
		}
	}

	void shiftRight(const AvxBitArray& bits, AvxBitArray& shiftedBits, bool leftmostBit) {
		shiftedBits = bits << 1;
		if (leftmostBit) {
			__m256i leftmost = _mm256_set1_epi32(LSB);
			shiftedBits.blend(leftmost, 0b00000001);
		}
	}

	void nextState(uint8_t** cells, int row, int column, BYTE* nextCells) {
		...
		shiftRight(neighbors[1], neighbors[0], (cells[row-1][column-1 ] & 0x80));	// top left
		shiftLeft (neighbors[1], neighbors[2], (cells[row-1][column+32] & 0x01));	// top right
		shiftRight(state,        neighbors[3], (cells[row  ][column-1 ] & 0x80));	// middle left
		shiftLeft (state,        neighbors[4], (cells[row  ][column+32] & 0x01));	// middle right
		shiftRight(neighbors[6], neighbors[5], (cells[row+1][column-1 ] & 0x80));	// bottom left
		shiftLeft (neighbors[6], neighbors[7], (cells[row+1][column+32] & 0x01));	// bottom right
		...
		result.write((AvxArray*)nextCells);
	}

	void drawPackedRLE(const char* rle, int x, int y, bool invertX, bool invertY, uint8_t** cells) {
		int charIndex = 0;
		int count = 0;
		int origX = x;
