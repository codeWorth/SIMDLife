...
struct ExploredSpace {
    ...
    ExploredSpace(): hitCount(0), height(0), success(false) {}
};

const int totalPairs = (NEIGHBOR_COUNT - 1) * NEIGHBOR_COUNT / 2;
const int usefulThreshold = 2;
    ...
    ExploredSpace findSwaps(
        ...
        iterCount++;
            ...
            if (iterCount >= 0xFFFFFF) {
                ...
                std::cout << "iter count = " << iterCount << std::endl;
        ...
        // if (iterCount >= 67108860) {
        //     iterCount = 0;
        //     uint16_t maxHits = 0;
        //     uint16_t minHits = -1;
        //     uint64_t uselessCount = 0;
        //     for (auto& it : exploredOutputSpaces) {
        //         if (!it.second.success && it.second.hitCount > maxHits) maxHits = it.second.hitCount;
        //         if (!it.second.success && it.second.hitCount < minHits) minHits = it.second.hitCount;
        //         if (!it.second.success && it.second.hitCount < usefulThreshold) uselessCount++;
        //     }
        //     std::cout << "Max hits: " << maxHits << std::endl;
        //     std::cout << "Min hits: " << minHits << std::endl;
        //     size_t oldSize = exploredOutputSpaces.size();
        //     double percent = round((double)uselessCount / oldSize * 100 * 1000) / 1000.0;
        //     std::cout << "Useless entries count: " << uselessCount << " (" << percent << "%)" << std::endl;
        //     std::cout << "Pruning, length = " << oldSize << std::endl;
        //     auto it = exploredOutputSpaces.begin();
        //     while (it != exploredOutputSpaces.end()) {
        //         if (!it->second.success && it->second.hitCount < usefulThreshold) {
        //             it = exploredOutputSpaces.erase(it);
        //         } else {
        //             it++;
        //         }
        //     }
        //     std::cout << "Finished pruning, length = " << exploredOutputSpaces.size() << std::endl;
        //     percent = round((double)exploredOutputSpaces.size() / oldSize * 100 * 1000) / 1000.0;
        //     std::cout << "Removed " << (oldSize - exploredOutputSpaces.size()) << " entries (now " << percent << "%)"  << std::endl;
        //     std::cout << std::endl;
        // }
        ...
        if (exploredIt != exploredOutputSpaces.end()) {
			...
			// we found a success (which is best possible) which will take too many swaps
            if (!explored.success && explored.height >= maxSwaps - swapsCount) {
                exploredIt->second.hitCount++;
                return exploredOut;
            }
            // we found a failure which took as many (or more) swaps as we have time for, therefore we cannot find a success for this state
            if (explored.success && swapsCount + explored.height > maxSwaps) return exploredOut;
