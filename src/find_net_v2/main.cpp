...
bool leaveTop4(bool* neighbors) {
    int bottomCount = 0;
    int topCount = 0;
    for (int i = 0; i < 4; i++) {
        if (neighbors[i]) bottomCount++;
    }
    for (int i = 4; i < 8; i++) {
        if (neighbors[i]) topCount++;
    }
    return bottomCount == 4 || topCount == 0;
}

int main(int argc, char *argv[]) {
    cout << "sizeof(AvxBitArray) = " << sizeof(AvxBitArray) << endl;
    cout << "sizeof(ExploredSpace) = " << sizeof(ExploredSpace) << endl;
    ...
    NetFinder finder(correctSwaps, 20, &maskFactory, ~allowedOutputSpace);
